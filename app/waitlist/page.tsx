...
export default function WaitlistPage() {
  ...
  const mrz = result ? `TF<${result.name.toUpperCase().replace(/\s+/g, '<')}<<<<<<<<<<<<<<<<<`.substring(0, 24) : 'TF<<<<<<<<<<<<<<<<<<<<<<<'
  ...
  return (
    <div style={{
      ...
      {/* Orbs */}
      <div style={{ position: 'absolute', width: 400, height: 400, borderRadius: '50%', top: -130, left: -90, background: 'radial-gradient(circle,rgba(255,100,60,0.22) 0%,transparent 68%)', filter: 'blur(50px)', pointerEvents: 'none' }} />
      <div style={{ position: 'absolute', width: 320, height: 320, borderRadius: '50%', bottom: -80, right: -80, background: 'radial-gradient(circle,rgba(160,60,255,0.2) 0%,transparent 70%)', filter: 'blur(48px)', pointerEvents: 'none' }} />
        ...
        {state === 'form' && (
          <>
            <div style={{ textAlign: 'center', marginBottom: 26, maxWidth: 320 }}>
              ...
              <p style={{ fontSize: 14, color: 'rgba(255,255,255,0.42)', lineHeight: 1.72, marginBottom: 8 }}>
                ...
                <span style={{ background: 'linear-gradient(135deg,#5fa8b8 0%,#4a8fa0 100%)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', fontWeight: 500 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, marginTop: 14, marginBottom: 28 }}>
                ...
                <span style={{ fontSize: 11, color: 'rgba(255,255,255,0.28)' }}>
                  <strong style={{ color: 'rgba(255,255,255,0.8)' }}>{memberCount}</strong> of 100 founding spots taken
                </span>
              </div>
            </div>

            <form onSubmit={handleSubmit} style={{ width: '100%', maxWidth: 340, background: 'rgba(180,180,190,0.08)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 28, padding: '26px 22px', backdropFilter: 'blur(32px)' }}>
              <label style={{ fontSize: 10, letterSpacing: '0.15em', color: 'rgba(255,255,255,0.3)', textTransform: 'uppercase', marginBottom: 8, display: 'block' }}>Your name</label>
              <input
                name="name" type="text" required
                placeholder="Your actual name (not your LLC)"
                style={{ display: 'block', width: '100%', background: 'rgba(160,165,175,0.1)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 25, padding: '14px 20px', fontSize: 15, color: '#ffffff', outline: 'none', marginBottom: 14 }}
              />
              <label style={{ fontSize: 10, letterSpacing: '0.15em', color: 'rgba(255,255,255,0.3)', textTransform: 'uppercase', marginBottom: 8, display: 'block' }}>Email address</label>
              <input
                name="email" type="email" required
                placeholder="you@example.com"
                style={{ display: 'block', width: '100%', background: 'rgba(160,165,175,0.1)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 25, padding: '14px 20px', fontSize: 15, color: '#ffffff', outline: 'none', marginBottom: 14 }}
              />
              {error && <p style={{ fontSize: 12, color: '#ff7a50', marginBottom: 10, textAlign: 'center' }}>{error}</p>}
              <button
                type="submit" disabled={loading}
                style={{ display: 'block', width: '100%', background: loading ? 'rgba(255,122,80,0.6)' : '#ff7a50', color: '#ffffff', border: 'none', borderRadius: 25, padding: '16px 20px', fontSize: 15, fontWeight: 500, cursor: loading ? 'not-allowed' : 'pointer', boxShadow: '0 8px 28px rgba(255,100,60,0.35)' }}
              >
          ...
          <>
            ...
            <div style={{ width: '100%', maxWidth: 340, position: 'relative', marginBottom: 16 }}>
              <div style={{
                ...
                {/* Neon border */}
                <div style={{ position: 'absolute', inset: 0, borderRadius: 28, padding: 1, background: 'linear-gradient(135deg,rgba(194,232,226,0.8) 0%,rgba(74,143,160,0.6) 50%,rgba(194,232,226,0.4) 100%)', WebkitMask: 'linear-gradient(#fff 0 0) content-box,linear-gradient(#fff 0 0)', WebkitMaskComposite: 'xor', pointerEvents: 'none' }} />
                ...
                <div style={{ position: 'relative', zIndex: 1, padding: '18px 22px 14px', borderBottom: '1px solid rgba(255,255,255,0.06)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  ...
                  <span style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 20, padding: '5px 11px', fontSize: 9, letterSpacing: '0.13em', color: 'rgba(255,255,255,0.6)', textTransform: 'uppercase', fontFamily: 'monospace' }}>Founding Member</span>
                </div>
                ...
                <div style={{ position: 'relative', zIndex: 1, padding: '20px 22px', display: 'flex', gap: 16 }}>
                  <div style={{ position: 'relative', width: 88, height: 88, flexShrink: 0 }}>
                    <svg width="88" height="88" viewBox="0 0 88 88">
                      ...
                      <circle cx="44" cy="44" r="42" fill="none" stroke="url(#rg)" strokeWidth="3.5" strokeLinecap="round"
                    ...
                    <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
                      <span style={{ fontFamily: 'monospace', fontSize: 19, fontWeight: 500, color: '#ffffff', lineHeight: 1 }}>{String(result.memberNumber).padStart(3, '0')}</span>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    {[
                      ...
                      { label: 'Issued', value: new Date().toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' }) },
                      ...
                      <div key={f.label} style={{ marginBottom: 12 }}>
                        ...
                        <p style={{ margin: '3px 0 0', fontSize: f.mono ? 11 : 13, color: f.mono ? 'rgba(255,255,255,0.6)' : '#ffffff', fontFamily: f.mono ? 'monospace' : 'inherit', letterSpacing: f.mono ? '0.06em' : 'normal', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{f.value}</p>
                      </div>
                    ))}
                  </div>
                </div>
                  ...
                  {[
                    ...
                    <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '5px 0' }}>
                      ...
                      <span style={{ fontSize: 12, color: 'rgba(255,255,255,0.42)' }}>
                        <strong style={{ color: '#ffffff', fontWeight: 500 }}>{b.split(' — ')[0]}</strong> — {b.split(' — ')[1]}
                      </span>
                    </div>
                  ))}
                </div>
            ...
            <button onClick={handleShare} style={{ display: 'block', width: '100%', maxWidth: 340, background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 25, padding: 14, fontSize: 13, color: 'rgba(255,255,255,0.6)', fontWeight: 500, cursor: 'pointer', marginBottom: 8 }}>
              Share your passport ↗
            </button>
          </>
        )}
